...
namespace archi
{
    namespace
    {
        ...
        const std::string& EditorSettingsPath()
        {
            static const std::string settingsPath = [] {
                ...
                if (const char* localAppData = std::getenv("LOCALAPPDATA"); localAppData && *localAppData)
                    settingsDirectory = std::filesystem::path(localAppData) / "ArchiEngine";
                else
                    settingsDirectory = std::filesystem::current_path() / ".archiengine";

                std::error_code error{};
                std::filesystem::create_directories(settingsDirectory, error);
                if (error)
                    return (std::filesystem::current_path() / "imgui.ini").string();

                const std::filesystem::path path = settingsDirectory / "imgui.ini";
                if (!std::filesystem::exists(path))
                {
                    const std::filesystem::path defaultLayout = ResolveAssetPath("editor/default_imgui.ini");
                    if (std::filesystem::exists(defaultLayout))
                        std::filesystem::copy_file(defaultLayout, path, std::filesystem::copy_options::skip_existing, error);
                }
        ...
        constexpr double kAssetCacheRefreshInterval = 1.0;
    ...
    bool EditorLayer::Init(IRenderAdapter& renderer)
    {
        ...
        io.IniFilename = EditorSettingsPath().c_str();
    ...
    void EditorLayer::RefreshAssetCache()
    {
        m_meshAssets = CollectAssets("models", { ".obj", ".fbx", ".gltf", ".glb", ".dae" });
        m_meshAssets.insert(m_meshAssets.begin(), "builtin:mesh:sphere");
        std::sort(m_meshAssets.begin(), m_meshAssets.end());
        m_meshAssets.erase(std::unique(m_meshAssets.begin(), m_meshAssets.end()), m_meshAssets.end());

        m_materialAssets = CollectAssets("materials", { ".json" });
        m_textureAssets = CollectAssets("textures", { ".png", ".jpg", ".jpeg", ".bmp", ".tga" });
        m_shaderAssets = CollectAssets("shaders", { ".json" });

        m_assetCacheRefreshAccumulator = 0.0;
        m_assetCacheInitialized = true;
    ...
    void EditorLayer::Build(Application& app, double dt)
    {
        ...
        m_assetCacheRefreshAccumulator += dt;
        if (!m_assetCacheInitialized || m_assetCacheRefreshAccumulator >= kAssetCacheRefreshInterval)
            RefreshAssetCache();
    ...
    void EditorLayer::DrawSelectedEntityInspector(Application& app, Entity entity)
    {
        ...
        if (auto* meshRenderer = world.GetComponent<MeshRenderer>(entity))
        {
            ...
            {
                editAssetCombo("Mesh", meshRenderer->meshAsset, m_meshAssets);

                if (editAssetCombo("Material", meshRenderer->materialAsset, m_materialAssets))
                ...
                editAssetCombo("Texture Override", meshRenderer->textureAsset, m_textureAssets);

                editAssetCombo("Shader Override", meshRenderer->shaderAsset, m_shaderAssets);
    ...
    void EditorLayer::DrawMaterialEditorPanel(Application& app)
    {
        ...
        if (m_selectedMaterialAsset.empty())
        {
            ...
            if (m_selectedMaterialAsset.empty() && !m_materialAssets.empty())
                m_selectedMaterialAsset = m_materialAssets.front();
        }
        ...
        ImGui::Separator();
        for (const std::string& materialAsset : m_materialAssets)
        {
        ...
        if (DrawAssetCombo("Shader", shaderAsset, m_shaderAssets) && shaderAsset != material.shaderAsset)
        {
        ...
        if (DrawAssetCombo("Texture", textureAsset, m_textureAssets) && textureAsset != material.textureAsset)
        {
        ...
        if (ImGui::RadioButton("Sphere", &m_materialPreviewMeshMode, 0))
            m_materialPreviewDirty = true;
        ImGui::SameLine();
        if (ImGui::RadioButton("Cube", &m_materialPreviewMeshMode, 1))
            m_materialPreviewDirty = true;
    ...
    void EditorLayer::DrawAssetBrowserPanel(Application& app)
    {
        ...
        drawAssetSection("Meshes", m_meshAssets, "ASSET_MESH", false);
        drawAssetSection("Materials", m_materialAssets, "ASSET_MATERIAL", true);
        drawAssetSection("Textures", m_textureAssets, "ASSET_TEXTURE", false);
        drawAssetSection("Shaders", m_shaderAssets, "ASSET_SHADER", false);
    ...
    void EditorLayer::RenderMaterialPreview(Application& app, double)
    {
        ...
        if (m_selectedMaterialAsset != m_lastPreviewMaterialAsset ||
            m_materialPreviewMeshMode != m_lastPreviewMeshMode ||
            m_materialPreviewWidth != m_lastPreviewWidth ||
            m_materialPreviewHeight != m_lastPreviewHeight)
        {
            m_materialPreviewDirty = true;
        }

        if (!m_materialPreviewDirty)
            return;
        ...
        const Mat4 model =
            MakeRotationYMatrix(Radians(30.0f)) *
        ...
        m_lastPreviewMaterialAsset = m_selectedMaterialAsset;
        m_lastPreviewMeshMode = m_materialPreviewMeshMode;
        m_lastPreviewWidth = m_materialPreviewWidth;
        m_lastPreviewHeight = m_materialPreviewHeight;
